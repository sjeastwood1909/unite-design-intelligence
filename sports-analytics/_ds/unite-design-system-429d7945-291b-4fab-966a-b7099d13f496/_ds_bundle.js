...
try { (() => {
function _extends() { return _extends = Object.assign ? Object.assign.bind() : function (n) { for (var e = 1; e < arguments.length; e++) { var t = arguments[e]; for (var r in t) ({}).hasOwnProperty.call(t, r) && (n[r] = t[r]); } return n; }, _extends.apply(null, arguments); }
/** Continuous or stepped range slider for numeric input. */
function Slider({
  label,
  min = 0,
  max = 100,
  ...
  return /*#__PURE__*/React.createElement("div", _extends({
    className: 'unite-slider ' + className,
    style: {
      width: '100%'
    }
  ...
  }, formatValue(current))), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      alignItems: 'center',
      gap: 8
  ...
  }, formatValue(min)), /*#__PURE__*/React.createElement("div", {
    style: {
      flex: 1,
      position: 'relative',
      height: 20,
    ...
    style: {
      ...
      width: pct + '%',
      height: 2,
      background: disabled ? 'var(--text-disabled)' : 'var(--interactive)'
    }
  }), /*#__PURE__*/React.createElement("div", {
    style: {
      position: 'absolute',
      ...
      boxShadow: '0 0 0 2px ' + (disabled ? 'var(--text-disabled)' : 'var(--interactive)'),
      pointerEvents: 'none'
    }
  ...
  }, formatValue(max))), (helperText || invalid && invalidText) && /*#__PURE__*/React.createElement("p", {
    style: {
      fontSize: 12,
      marginTop: 6,
      color: invalid ? 'var(--text-error)' : 'var(--text-helper)'
    }
...
const COLORS = ['var(--blue-60)', 'var(--teal-60)', 'var(--cyan-50)', 'var(--purple-60)', 'var(--magenta-60)', 'var(--green-50)', 'var(--orange-50)'];
...
/** Renders a user avatar — image or generated initials. */
function Avatar({
  name,
  src,
  size = 'md',
  ...
  const fontSize = px <= 24 ? 10 : px <= 32 ? 12 : px <= 40 ? 14 : 22;
  const borderRadius = shape === 'circle' ? '50%' : shape === 'rounded' ? 'var(--radius-md)' : 0;
  return /*#__PURE__*/React.createElement("span", _extends({
    className: 'unite-avatar unite-avatar--' + size + ' ' + className,
    title: name,
    style: {
      display: 'inline-flex',
      alignItems: 'center',
...
// components/data/StructuredList.jsx
try { (() => {
function _extends() { return _extends = Object.assign ? Object.assign.bind() : function (n) { for (var e = 1; e < arguments.length; e++) { var t = arguments[e]; for (var r in t) ({}).hasOwnProperty.call(t, r) && (n[r] = t[r]); } return n; }, _extends.apply(null, arguments); }
/** Lightweight two-column data list — use instead of DataTable for simple key/value or short tabular data. */
function StructuredList({
  ...
  return /*#__PURE__*/React.createElement("div", _extends({
    className: 'unite-structured-list ' + className,
    style: {
      width: '100%',
      border: border ? '1px solid var(--border-subtle-01)' : 'none'
    }
  }, rest), headers && headers.length > 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'grid',
  ...
  }, headers.map((h, i) => /*#__PURE__*/React.createElement("span", {
    key: i,
    style: {
      padding: '8px 16px',
      fontSize: 12,
    ...
    const key = row.key ?? ri;
    const isSel = selection && selected === key;
    return /*#__PURE__*/React.createElement("div", {
      key: key,
      onClick: selection ? () => setSelected(isSel ? null : key) : undefined,
      style: {
        display: 'grid',
        gridTemplateColumns: `repeat(${cells.length}, 1fr)`,
        borderBottom: ri < rows.length - 1 ? '1px solid var(--border-subtle-01)' : 'none',
        background: isSel ? 'var(--layer-selected-01)' : 'transparent',
        boxShadow: isSel ? 'inset 2px 0 0 var(--interactive)' : 'none',
        cursor: selection ? 'pointer' : 'default',
        transition: 'background var(--duration-fast-01) var(--easing-standard-productive)'
      },
      onMouseEnter: e => {
        if (!isSel) e.currentTarget.style.background = 'var(--background-hover)';
      },
      onMouseLeave: e => {
        if (!isSel) e.currentTarget.style.background = 'transparent';
      }
    }, cells.map((cell, ci) => /*#__PURE__*/React.createElement("span", {
      key: ci,
      style: {
        padding: '12px 16px',
        fontSize: 14,
        ...
        fontFamily: typeof cell === 'number' ? 'var(--font-mono)' : 'var(--font-sans)'
      }
    }, cell)));
  }));
}
Object.assign(__ds_scope, { StructuredList });
})(); } catch (e) { __ds_ns.__errors.push({ path: "components/data/StructuredList.jsx", error: String((e && e.message) || e) }); }
...
function _extends() { return _extends = Object.assign ? Object.assign.bind() : function (n) { for (var e = 1; e < arguments.length; e++) { var t = arguments[e]; for (var r in t) ({}).hasOwnProperty.call(t, r) && (n[r] = t[r]); } return n; }, _extends.apply(null, arguments); }
/**
 * ComboBox — a filterable dropdown. Users type to narrow the list;
 * the selected value replaces the input text.
 */
function ComboBox({
  label,
  id,
  ...
  const [internalVal, setInternalVal] = React.useState(defaultValue ?? null);
  const ref = React.useRef(null);
  const inputRef = React.useRef(null);
  const controlled = value !== undefined;
  const current = controlled ? value : internalVal;
  const normalize = item => ({
    label: typeof item === 'string' ? item : item.label,
    value: typeof item === 'string' ? item : item.value ?? item.label,
    disabled: typeof item === 'object' ? item.disabled : false
  });
  const filtered = items.map(normalize).filter(it => it.label.toLowerCase().includes(inputVal.toLowerCase()));
  React.useEffect(() => {
    if (!open) return;
    const handler = e => {
  ...
  const currentItem = items.map(normalize).find(it => it.value === current);
  const displayLabel = open ? inputVal : currentItem?.label ?? '';
  const select = item => {
    if (!controlled) setInternalVal(item.value);
    setInputVal('');
    setOpen(false);
  ...
  const clear = e => {
    e.stopPropagation();
    if (!controlled) setInternalVal(null);
    setInputVal('');
    onChange?.(null, null);
    inputRef.current?.focus();
  };
  const inputId = id || 'unite-combo-' + Math.random().toString(36).slice(2);
  return /*#__PURE__*/React.createElement("div", _extends({
    ref: ref,
    className: 'unite-combobox ' + className,
    style: {
      width: '100%',
      position: 'relative'
    }
    ...
    style: {
      ...
      border: 'none',
      borderBottom: invalid ? '2px solid var(--support-error)' : open ? '2px solid var(--focus)' : '1px solid var(--border-strong-01)',
      color: disabled ? 'var(--text-disabled)' : 'var(--text-primary)',
      fontSize: 14,
      fontFamily: 'var(--font-sans)',
      outline: 'none',
  ...
  }))))), open && filtered.length > 0 && /*#__PURE__*/React.createElement("ul", {
    style: {
      position: 'absolute',
      zIndex: 9000,
      top: '100%',
    ...
    onMouseDown: () => !item.disabled && select(item),
    ...
    style: {
      ...
      padding: '10px 16px',
      border: 'none',
      background: item.value === current ? 'var(--layer-selected-01)' : 'transparent',
      cursor: item.disabled ? 'not-allowed' : 'pointer',
      fontSize: 14,
      color: item.disabled ? 'var(--text-disabled)' : 'var(--text-primary)',
      fontFamily: 'var(--font-sans)',
      boxShadow: item.value === current ? 'inset 2px 0 0 var(--interactive)' : 'none'
    },
    onMouseEnter: e => {
      if (item.value !== current && !item.disabled) e.currentTarget.style.background = 'var(--background-hover)';
    },
    onMouseLeave: e => {
      if (item.value !== current) e.currentTarget.style.background = 'transparent';
    }
  }, item.label)))), open && filtered.length === 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      position: 'absolute',
      zIndex: 9000,
      top: '100%',
  ...
  }, "No results found"), (helperText || invalid && invalidText) && /*#__PURE__*/React.createElement("p", {
    style: {
      fontSize: 12,
      marginTop: 4,
      color: invalid ? 'var(--text-error)' : 'var(--text-helper)'
...
/** DatePicker — calendar popup triggered by a text input. */
function DatePicker({
  label,
  id,
  ...
  const [view, setView] = React.useState(() => {
    const d = defaultValue ? new Date(defaultValue) : today;
    return new Date(d.getFullYear(), d.getMonth(), 1);
  });
  const [internalVal, setInternalVal] = React.useState(defaultValue ? new Date(defaultValue) : null);
  const ref = React.useRef(null);
  const controlled = value !== undefined;
  const current = controlled ? value ? new Date(value) : null : internalVal;
  React.useEffect(() => {
    if (!open) return;
    const handler = e => {
      if (ref.current && !ref.current.contains(e.target)) setOpen(false);
  ...
  const selectDay = day => {
    ...
    if (!controlled) setInternalVal(d);
    setOpen(false);
    onChange?.(d, formatDate(d));
  };
  const inputId = id || 'unite-datepicker-' + Math.random().toString(36).slice(2);
  return /*#__PURE__*/React.createElement("div", _extends({
    ref: ref,
    className: 'unite-datepicker ' + className,
    style: {
      width: '100%',
      position: 'relative'
    }
    ...
    d: "M13 2h-1V1h-1v1H5V1H4v1H3a1 1 0 00-1 1v10a1 1 0 001 1h10a1 1 0 001-1V3a1 1 0 00-1-1zm0 11H3V6h10v7zm0-8H3V3h1v1h1V3h6v1h1V3h1v1z"
    ...
    d: "M10 12.7L5.3 8 10 3.3l-.7-.7L4 8l5.3 5.3z"
  }))), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 14,
      fontWeight: 600,
  ...
  }, MONTH_NAMES[view.getMonth()], " ", view.getFullYear()), /*#__PURE__*/React.createElement("button", {
    onClick: nextMonth,
    ...
    d: "M6 3.3L10.7 8 6 12.7l.7.7L12 8 6.7 2.7z"
  })))), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'grid',
    ...
    const isSelected = current && current.getTime() === date.getTime();
    ...
    return /*#__PURE__*/React.createElement("button", {
      ...
      style: {
        width: '100%',
        aspectRatio: '1',
        border: !isSelected && isToday ? '1px solid var(--interactive)' : 'none',
        background: isSelected ? 'var(--interactive)' : 'transparent',
      ...
      onMouseEnter: e => {
        if (!isSelected) e.currentTarget.style.background = 'var(--background-hover)';
      },
      onMouseLeave: e => {
        if (!isSelected) e.currentTarget.style.background = 'transparent';
      }
    }, day);
  }))), (helperText || invalid && invalidText) && /*#__PURE__*/React.createElement("p", {
    style: {
      fontSize: 12,
      marginTop: 4,
      color: invalid ? 'var(--text-error)' : 'var(--text-helper)'
...
try { (() => {
function _extends() { return _extends = Object.assign ? Object.assign.bind() : function (n) { for (var e = 1; e < arguments.length; e++) { var t = arguments[e]; for (var r in t) ({}).hasOwnProperty.call(t, r) && (n[r] = t[r]); } return n; }, _extends.apply(null, arguments); }
/** Multi-select dropdown — allows selecting multiple items, displays a count badge. */
function MultiSelect({
  ...
  const toggle = val => {
    ...
    if (!controlled) setInternalVal(next);
    onChange?.(next);
  };
  const clearAll = e => {
    e.stopPropagation();
    if (!controlled) setInternalVal([]);
    onChange?.([]);
  };
  const inputId = id || 'unite-multiselect-' + Math.random().toString(36).slice(2);
  return /*#__PURE__*/React.createElement("div", _extends({
    ref: ref,
    className: 'unite-multiselect ' + className,
    style: {
      width: '100%',
      position: 'relative'
    }
    ...
    style: {
      ...
      border: 'none',
      borderBottom: invalid ? '2px solid var(--support-error)' : open ? '2px solid var(--focus)' : '1px solid var(--border-strong-01)',
      color: current.length ? 'var(--text-primary)' : 'var(--text-placeholder)',
      fontSize: 14,
      fontFamily: 'var(--font-sans)',
      textAlign: 'left',
      cursor: disabled ? 'not-allowed' : 'pointer'
  ...
  }, current.length ? `${current.length} item${current.length !== 1 ? 's' : ''} selected` : placeholder), current.length > 0 && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("span", {
    style: {
      display: 'flex',
      alignItems: 'center',
      justifyContent: 'center',
  ...
  }, current.length), /*#__PURE__*/React.createElement("button", {
    onClick: clearAll,
    style: {
      width: 32,
      height: '100%',
    ...
    return /*#__PURE__*/React.createElement("li", {
      ...
      onMouseDown: () => !item.disabled && toggle(item.value),
      disabled: item.disabled,
      style: {
        display: 'flex',
        alignItems: 'center',
        ...
        padding: '10px 16px',
        border: 'none',
        background: checked ? 'var(--layer-selected-01)' : 'transparent',
        cursor: item.disabled ? 'not-allowed' : 'pointer',
        fontSize: 14,
        color: item.disabled ? 'var(--text-disabled)' : 'var(--text-primary)',
        fontFamily: 'var(--font-sans)',
        boxShadow: checked ? 'inset 2px 0 0 var(--interactive)' : 'none'
      },
      onMouseEnter: e => {
        if (!checked && !item.disabled) e.currentTarget.style.background = 'var(--background-hover)';
      },
      onMouseLeave: e => {
      ...
      style: {
        ...
        height: 16,
        flexShrink: 0,
        border: checked ? 'none' : '1px solid var(--icon-primary)',
        background: checked ? 'var(--interactive)' : 'transparent',
        display: 'flex',
        alignItems: 'center',
        justifyContent: 'center'
      }
...
function _extends() { return _extends = Object.assign ? Object.assign.bind() : function (n) { for (var e = 1; e < arguments.length; e++) { var t = arguments[e]; for (var r in t) ({}).hasOwnProperty.call(t, r) && (n[r] = t[r]); } return n; }, _extends.apply(null, arguments); }
/**
 * Overflow menu — a vertically-aligned "⋮" button that reveals a floating
 * action list. Use in table rows, toolbars, cards, and panel headers.
 */
function OverflowMenu({
  ...
  const top = align.startsWith('bottom') ? '100%' : 'auto';
  const bottom = align.startsWith('top') ? '100%' : 'auto';
  const right = align.endsWith('right') ? 0 : 'auto';
  const left = align.endsWith('left') ? 0 : 'auto';
  return /*#__PURE__*/React.createElement("div", _extends({
    ref: ref,
    className: 'unite-overflow-menu ' + className,
    style: {
      position: 'relative',
      display: 'inline-block'
    }
    ...
    style: {
      ...
      height: px,
      padding: 0,
      background: open ? 'var(--background-selected)' : 'transparent',
      border: 'none',
      cursor: 'pointer',
      color: 'var(--icon-primary)',
      transition: 'background var(--duration-fast-01) var(--easing-standard-productive)'
    ...
    onMouseEnter: e => {
      if (!open) e.currentTarget.style.background = 'var(--background-hover)';
    },
    onMouseLeave: e => {
  ...
  }, items.map((item, i) => item.divider ? /*#__PURE__*/React.createElement("li", {
    key: i,
    role: "separator",
    style: {
    ...
    style: {
      ...
      background: 'transparent',
      cursor: item.disabled ? 'not-allowed' : 'pointer',
      color: item.danger ? 'var(--support-error)' : item.disabled ? 'var(--text-disabled)' : 'var(--text-primary)',
      fontSize: 14,
      fontFamily: 'var(--font-sans)',
      textAlign: 'left'
    },
    onMouseEnter: e => {
      if (!item.disabled) e.currentTarget.style.background = 'var(--background-hover)';
    },
    onMouseLeave: e => {
    ...
    style: {
      color: item.danger ? 'var(--support-error)' : 'var(--icon-secondary)',
      display: 'flex'
    }
...
function Wordmark({
  ...
  // New logo is light-coloured by default (white text + blue icon) — suits dark headers.
  // On a light background pass light={true} to darken the wordmark text to black.
...
__ds_ns.StructuredList = __ds_scope.StructuredList;
